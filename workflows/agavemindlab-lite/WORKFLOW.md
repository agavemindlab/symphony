...
hooks:
  ...
  before_turn: |
    ...
    "$workflow_source_dir/../agavemindlab/snapshot-shared-skills.sh" "$project_workflow_dir/skills"
  before_remove: |
    set -e
    : "${SYMPHONY_WORKFLOW_DIR:?SYMPHONY_WORKFLOW_DIR is not set}"
    if [ -f "$SYMPHONY_WORKFLOW_DIR/project-for-linear-project.sh" ]; then
...
- 需要人类决定时，写可见的 `### NEEDS CLARIFICATION` block，问题正文默认展开，不放进折叠块；给出你的推荐默认值和影响。
