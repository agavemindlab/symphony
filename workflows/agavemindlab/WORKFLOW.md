...
| Design | `phase-design` | Main Flow confirmed Requirements has a closing reply |
| Implementation | `phase-implementation` | Main Flow confirmed Design has a closing reply |
| Deployment | `phase-deployment` | Human approves merge (`Merging` state) |

Requirements and Design clean exits always hand back `stop`: Main Flow atomically
moves the issue to `Human Review` with `symphony:maestro`, then waits for Maestro
or a human to approve. When that approval moves the issue back to `In Progress`,
the next Symphony dispatch writes the `✅ 已批准，进入 [Next Phase]` closing
reply and continues. **Implementation** always stops at `Human Review` (the PR
is up), and **Deployment** is reachable only via `Merging`.
...
5. Determine intent:
   ...
   **Completed Maestro transition cleanup — run before every intent rule.** When
   `symphony:maestro` remains on an active issue, remove `symphony:maestro` before intent routing.
   If removal fails, retry once and stop before intent routing on a second failure.

   **If the awaiting-review artifact still carries an unresolved clarification gate** (`### NEEDS CLARIFICATION`, or the legacy `[NEEDS CLARIFICATION]` marker) (the phase published a clarification question rather than ordinary review), this branch takes precedence over every intent rule below. With no new human reply, never approve or close the artifact. Evaluate it in this order: only a complete Requirements or Design gate uses the direct blocked exit below; otherwise, if a qualifying auto-rework signal exists, route its `/rework <phase>` draft literally under the rule below; otherwise target the current phase, continue to step 6, and let the phase skill's incomplete-gate exit return `stop` for the standard handoff; do not stop in this intent branch. A new human reply in its thread is an **answer to that question**, not an approval or a new change request. Target phase = the current (awaiting-review) phase; do **not** write an approval reply. Re-open that phase's skill, which follows its own "On resume" path: fold each answer into a revised artifact, drop the resolved gate, and decide the phase exit again. When the revised artifact needs review, publication follows the same-phase Rework cycle even if the Linear state is `In Progress`: resolve the old artifact, post a fresh top-level artifact, and put the clarification summary on the new artifact; do not `commentUpdate` the old artifact. If an answer does not actually resolve a gate, the skill keeps it open, refines the question, and stops again (its "When blocked" / "On resume" defines the re-ask and the two-round escalation).
...
6. Set the workpad `current_phase` to the target phase and open the matching phase skill (per the Phase Map). The skill does its phase work, publishes its artifact through the Phase Artifact Protocol, and on completion hands back `stop`.

   - **`stop`** → for Implementation, include `symphony:maestro` when `Review verdict` is exactly `CLEAN` or `ESCALATED`, and for no-PR `Type:Spike` findings; for an absent or malformed verdict on PR-producing work, ensure the label is absent. For every eligible outcome above and for other phases, use one `issueUpdate` mutation that sets the `Human Review` state and full label set together, preserving existing labels and including `symphony:maestro`. If it fails, retry that mutation once; on a second failure, record the exact blocker in the status card and stop. Then stop.

   (Only a complete unresolved Requirements or Design clarification gate uses the direct blocked exit. When the current awaiting Requirements or Design artifact carries that gate, at least one unresolved question exists, and every unresolved question includes a concrete question, why only a human can decide, at least two distinct, nonblank concrete answer options, exactly one recommended answer per question, and the consequence of every option, move the issue to `Human Review`, leave `symphony:maestro` unchanged, start no Maestro session, and keep the status card pointed at the original artifact thread instead of posting a second summary. Immediately before the direct `Human Review` state update, re-read the artifact thread and issue state; if either changed, discard the stale action and route the fresh signals instead. State that the human answers in that thread and then moves the issue to `In Progress`; the agent stops with the issue in `Human Review`. Retry that `Human Review` state update once; on a second failure, record the exact blocker in the status card and stop. An incomplete gate follows the existing review/rework handoff by returning `stop`; when a fresh revised artifact no longer has an unresolved clarification gate, resume the standard Maestro route. Other blocked outcomes follow their skill's handling.)
...
A phase artifact is **closed** (no longer awaiting review) once its thread carries a Main-Flow-written closing reply:

- `✅ 已批准，进入 [Next Phase]（[timestamp]）` — **review approval**. Main Flow writes it (step 5, or the `Merging` branch of step 3) after human or qualifying Maestro approval.

Historical `⏩ 自动进入` replies count as closing replies for routing but are never written on new artifacts.
...
- **Phase gating**: phase advancement is driven by human signals (Linear state + human words) and Maestro preflight, not by the phase agent that authored the artifact. A bot / automated PR review (e.g. `AUTOMATED_REVIEWER`) is never an approval or a phase-routing signal; its feedback is handled inside the Implementation PR feedback sweep.
- **Requirements/Design review**: Requirements and Design clean exits always stop at `Human Review` with `symphony:maestro`; Maestro may auto-approve by moving the issue back to `In Progress`, and the next Main Flow run writes the `✅` closing reply. Implementation waits for `Merging` before Deployment.
...
- **`Human Review` is not an agent state for the normal workflow**: the normal workflow does not start there. The separate `MAESTRO_WORKFLOW.md` may watch `Human Review` only with both `symphony` and `symphony:maestro` labels. It removes the label when review stays in `Human Review`; Main Flow consumes it after an auto-transition.
