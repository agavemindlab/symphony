...
- **`Human Review` is not an agent state**: Symphony does not start the agent there. Do not design any phase skill to act while the issue is in `Human Review`. Whenever a run moves the issue to `Human Review`, add the `maestro:review` label in the same operation (create the label once if the team lacks it) — it is the dispatch gate for the standalone Maestro reviewer instance, which removes it after reviewing.
