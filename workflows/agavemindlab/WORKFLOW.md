...
6. Set the workpad `current_phase` to the target phase and open the matching phase skill (per the Phase Map). The skill does its phase work, publishes its artifact through the Phase Artifact Protocol, and on completion hands back one of two outcomes — the skill alone decides which (see its "Exit"); only the Requirements and Design skills ever choose `advance`:
   ...
   (A Requirements or Design skill that stops **blocked** on an unresolved `### NEEDS CLARIFICATION` or legacy marker moves the issue to `Human Review` itself. When the current awaiting Requirements or Design artifact carries such a gate and every unresolved question includes a concrete question, why only a human can decide, at least two distinct, nonblank concrete answer options, exactly one recommended answer per question, and the consequence of every option, leave `symphony:maestro` unchanged, start no Maestro session, keep the status card pointed at the original artifact thread instead of posting a second summary, and end the session there. An incomplete gate follows the existing review/rework handoff; when a fresh revised artifact no longer has an unresolved clarification gate, resume the standard Maestro route. Other blocked outcomes follow their skill's handling.)
