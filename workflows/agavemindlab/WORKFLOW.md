...
Confine all **writes** to the provided repository copy, with one exception: the Implementation phase's phase-required gstack `review` may write local runtime state under `$HOME/.gstack/`, managed nested-Codex session state under `$HOME/.codex/`, and create and clean `/tmp/codex-adv-*` and `/tmp/codex-review-*`. This exception does not authorize another gstack skill, any other `$HOME`, system, or `/tmp` path, another checkout, or another external system.
