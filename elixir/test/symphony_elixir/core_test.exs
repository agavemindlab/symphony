defmodule SymphonyElixir.CoreTest do
  ...
  test "agent runner skips run hooks when launch identity no longer matches" do
    test_root =
      Path.join(System.tmp_dir!(), "symphony-elixir-agent-runner-identity-change-#{System.unique_integer([:positive])}")
    ...
    try do
      ...
      counter = Path.join(test_root, "selector.count")
      workspace_root = Path.join(test_root, "workspaces")
      ...
      if [ "$count" -eq 1 ]; then SYMPHONY_REPO="symphony"; else SYMPHONY_REPO="grotto"; fi
      """)

      write_workflow_file!(workflow_file,
        workspace_root: workspace_root,
        hook_after_create: "echo created > created.txt",
        hook_before_run: "echo before > before-run.txt",
        hook_after_run: "echo after > after-run.txt"
      )

      issue = %Issue{
        id: "issue-agent-identity-change",
        identifier: "MT-AGENT-IDENTITY-CHANGE",
        title: "Identity changes before launch",
        state: "In Progress",
      ...
      assert File.exists?(Path.join(workspace, "created.txt"))
      refute File.exists?(Path.join(workspace, "before-run.txt"))
